...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function estimateTextHalfHeightPercent(card, textItem) {
    ...
    const paddingPx = (Number(textItem?.background?.opacity || 0) > 0)
      ? Number(textItem?.background?.paddingY || 0) * 2
      : 0;
  ...
  function alignSelectedTexts(mode) {
    ...
    const targets = getSelectedTexts(card);
    if (targets.length < 2) {
      setStatus('Shift+클릭으로 텍스트 2개 이상 선택 후 정렬하세요.', 'info');
      return;
    }
  ...
  const MOBILE_PANEL_CLASS_MAP = {
    cards: 'is-mobile-step-cards',
    format: 'is-mobile-step-format',
    text: 'is-mobile-step-text',
    background: 'is-mobile-step-background',
    image: 'is-mobile-step-image',
    shape: 'is-mobile-step-shape',
    layer: 'is-mobile-step-layer',
    export: 'is-mobile-step-export'
  };
  ...
  function getMobilePanelFromSection(sectionKey = '') {
    return MOBILE_PANEL_CLASS_MAP[sectionKey] ? sectionKey : 'cards';
  }
  ...
  function applyMobileLayoutOverrides(isMobile) {
    ...
    if (!isMobile) {
      ...
      if (mobileUi.quickbar) {
        ...
        mobileUi.quickbar.style.removeProperty('margin-top');
      }
      return;
    }
    ...
    if (mobileUi.quickbar) {
      ...
      mobileUi.quickbar.style.setProperty('margin-top', '0', 'important');
    }
